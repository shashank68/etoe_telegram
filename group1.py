...
class InteractiveTelegramClient(TelegramClient):
        ...
        while True:
            ...
            while True:
                ...
                if msg:
                    ...
                    if type(entity) == telethon.tl.types.Chat:

                        async for person in self.iter_participants(entity):
                            if (person.is_self == False):
                                print(person.id)
                                aes_shared_key = None
                                for dlg in Dialog.select():
                                    if dlg.dialog_id == person.id:
                                ...
                                if aes_shared_key is None:
                                    # get the public key.
                                    peer_pub_key = get_public_key(person.id)
                                    ...
                                    peer = Dialog(
                                        dialog_id=person.id, aes_shared_key=aes_shared_key
                                    )
    ...
    async def message_handler(self, event):
        ...
        if event.text:
            ...
            if (chat[0:10] == "0000000000" or chat[0:10] == me.id):
                if event.is_group:
                    sprint(
                        '<< {} @ {} sent "{}"'.format(
                            get_display_name(await event.get_sender()),
                            get_display_name(chat[10:]),
                            event.text,
                        )
                    )
                else:

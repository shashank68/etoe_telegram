...
class InteractiveTelegramClient(TelegramClient):
    ...
    async def run(self):
        ...
        while True:
            ...
            while True:
                ...
                if InteractiveTelegramClient.shared_key != None:
                    l = len(InteractiveTelegramClient.shared_key)
                    key = AES.new(bytes(InteractiveTelegramClient.shared_key, 'utf-8')[:32], AES.MODE_ECB)
                    InteractiveTelegramClient.aeskey = key
                # Quit
                if msg == '!q':
                    break
                elif msg == '!Q':
                ...
                # Send chat message (if any)
                    ...
                    # print(entity)
                    if InteractiveTelegramClient.aeskey != None:
                        print("Inside")
                        await self.send_message(entity, InteractiveTelegramClient.aeskey.encrypt(msg), link_preview=False)
                    else:
    ...
    async def message_handler(self, event):
        ...
        if event.text[:9] == '__PUB_KEY':
            ...
            text = InteractiveTelegramClient.aeskey.decrypt(event.text)
            print(text)
